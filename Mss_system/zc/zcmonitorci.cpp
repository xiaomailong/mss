...
ZCMonitorCI::ZCMonitorCI(QWidget *parent,ZCReadData *zcReadCiData) :
    QWidget(parent),
    ui(new Ui::ZCMonitorCI)
{
    ui->setupUi(this);
    //set current Page
    packageIndex=0;
    ...
    connect(this,&ZCMonitorCI::sendEventToRead,zcReadCiData,&ZCReadData::DealFromZCCi);
    connect(zcReadCiData,&ZCReadData::sendDataToZCCi,this,&ZCMonitorCI::onEventFromRead);
    on_queryButton_clicked();
}

ZCMonitorCI::~ZCMonitorCI()
{
    delete ui;
}
//receive data from zcreadData
void ZCMonitorCI::onEventFromRead(QSqlTableModel *tableModel)
{
    model=tableModel;
    if(isGetCount){
        totalRecrodCount=model->rowCount();
        getTotalPage();
        qDebug()<<"ZCMonitorCI totalPage :"<<totalPage;
        updateStatus();
        isGetCount=false;
        findData();
        return;
    }
    qDebug()<<"ZCMonitorCI onDataFromRead :"<<model->rowCount();
...
void ZCMonitorCI::on_queryButton_clicked()
{
    isGetCount=true;
    findData();
}

void ZCMonitorCI::on_prevButton_clicked()
{
    currentPage--;
    findData();
    updateStatus();
...
void ZCMonitorCI::on_nextButton_clicked()
{
    currentPage++;
    findData();
    updateStatus();
...
void ZCMonitorCI::on_switchPageButton_clicked()
{
...
void ZCMonitorCI::on_filter_cbo_currentIndexChanged(int index)
{
     packageIndex=index;
}
void ZCMonitorCI::on_startDateEdit_dateTimeChanged(const QDateTime &dateTime)
{
    startDateTime = dateTime;
}

void ZCMonitorCI::on_endDateEdit_dateTimeChanged(const QDateTime &dateTime)
{
    endDateTime = dateTime;
}

void ZCMonitorCI::on_comboBox_currentIndexChanged(int index)
{
    switch (index) 
    {
    case 0:
...
void ZCMonitorCI::on_pushButton_clicked()
{
    getTotalPage();
    findData();
    updateStatus();
